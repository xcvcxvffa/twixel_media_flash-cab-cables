...
export async function apiUpdateProfile<T, U extends Record<string, unknown>>(data: U) {
    return ApiService.fetchData<T>({
        url: '/account/profile',
        method: 'post',
        data,
...
export async function apiUpdatePassword<T, U extends Record<string, unknown>>(data: U) {
    return ApiService.fetchData<T>({
